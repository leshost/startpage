...
2. **Налаштуйте базу даних:**
   Створіть нову базу даних у MySQL та імпортуйте таблицю за допомогою SQL-коду з файлу `SQL.txt`:
   ```sql
   CREATE TABLE `sites` (
...
3. **Налаштуйте підключення (`config.php`):**
   Відкрийте файл `config.php` і вкажіть свої дані для доступу до бази даних, а також змініть пароль адміністратора (`ADMIN_PASSWORD`).
   ```php
   define('DB_HOST', 'localhost');
   define('DB_NAME', 'ваша_база_даних');
   define('DB_USER', 'користувач_бд');
   define('DB_PASS', 'пароль_бд');
   define('ADMIN_PASSWORD', 'свій_секретний_пароль');
   ```
